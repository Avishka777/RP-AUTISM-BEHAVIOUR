...
async function generatePDF(session, takenTimeChartBuffer, isCorrectChartBuffer) {
  return new Promise((resolve, reject) => {
    ...
    const sinhalaFontPath = path.join(__dirname, "../Noto_Sans_Sinhala/NotoSansSinhala-VariableFont_wdth,wght.ttf");
    if (fs.existsSync(sinhalaFontPath)) {
      doc.registerFont("Sinhala", sinhalaFontPath);
      doc.font("Sinhala");
    } else {
      console.error("Sinhala font not found at " + sinhalaFontPath + ". Using default font.");
      doc.font("Helvetica");
    }
    ...
    doc.fontSize(16).fillColor("black").text("User Details", { underline: true });
    doc.moveDown();
    doc.fontSize(12)
    ...
    doc.fontSize(12)
      ...
      .text(`Current Mood (Emotions When Doing Activity): ${session.currentMood}`)
    ...
    doc.fontSize(16).text("Instruction Records", { underline: true });
    doc.moveDown();
    ...
    doc.addPage();
    doc.fontSize(16).text("Quiz Taken Time Chart", { align: "center" });
    doc.moveDown();
    doc.image(takenTimeChartBuffer, { fit: [500, 300], align: "center" });

    // Add a new page for the second chart
    doc.addPage();
    doc.fontSize(16).text("Quiz Correctness Chart", { align: "center" });
    doc.moveDown();
...
exports.sendSessionEmail = async (req, res) => {
  try {
    ...
    const labels = session.InstructinRecords.map(record => record.instructioId);
    const takenTimes = session.InstructinRecords.map(record => record.takenTime);
    ...
    const takenTimeChartUrl = `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify(takenTimeChartConfig))}`;

    // Chart 2: Correctness per Instruction (Bar Chart)
    const correctnessLabels = session.InstructinRecords.map(record => record.instructioId);
    const correctnessData = session.InstructinRecords.map(record => record.isCorrect ? 1 : 0);
    const isCorrectChartConfig = {
      ...
      data: {
        ...
        datasets: [{
          ...
          backgroundColor: correctnessData.map(val => (val === 1 ? "green" : "red"))
        }]
      },
      options: {
    ...
    const isCorrectChartUrl = `https://quickchart.io/chart?c=${encodeURIComponent(JSON.stringify(isCorrectChartConfig))}`;

    // --- Download Chart Images as Buffers ---
    const takenTimeChartResponse = await axios.get(takenTimeChartUrl, { responseType: "arraybuffer" });
    const isCorrectChartResponse = await axios.get(isCorrectChartUrl, { responseType: "arraybuffer" });
    ...
    const pdfPath = await generatePDF(session, takenTimeChartBuffer, isCorrectChartBuffer);
